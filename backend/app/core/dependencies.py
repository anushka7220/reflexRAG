#DRY (Don't Repeat Yourself), every endpoint needs a validation, so logic will repeat everytime.
#FastAPI has a built-in system called dependency injection.
#You write the logic once as a function, then declare it as a dependency on any endpoint that needs it.
# FastAPI runs it automatically before your endpoint code, and injects the result as a parameter.
